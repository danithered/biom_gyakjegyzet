...
Amivel foglalkozni tudunk, az két, a gyakorlat során gyakran használt operátor: a *szumma* és a *produktum*.

## A *szumma* operátor

Ha egy hosszabb sorozat tagjainak az összegét akarjuk kiszámolni, felírhatjuk a megfelelő egyenletet a sorozat tagjainak felsorolásával is, mint:
$$ x_1 + x_2 + x_3 + ... +x_n $$
, ahol egy sorozat tagjait összeadjuk az első tagtól az *n*-edikig. Azonban a *szumma* ($\sum$) operátor erre egy sokkal egyszerűbb és kényelmesebb megoldást nyújt. A *szumma* operátor után következő kifejezés megadja az összeadandó produktum tagjait, míg az operátor alatt és felett sorra megadhatjuk egy új változó segítségével, hogy a sorozat mely tagjait kívánjuk összeadni. Például az előző kifejezés a *szumma* operátor segítségével leírva így nézne ki:
$$ \sum_{i=1}^{n}{x_i} = x_1 + x_2 + x_3 + \dots +x_n $$
Itt az $x_i$ jelöli a sorozat tagjait, az $i$ pedig egy új változó, amit a *szumma* jel alatt és felett definiáltunk. A *szumma* jel alatt szoktuk definiálni az új változó kezdeti értékét, felette pedig az utolsó értékét. Itt például az $i$ 1-től $n$-ig fut: 1, 2, 3, ..., $n$. Tegyük fel, hogy $x$ egy vektor:
...
Ez úgy jön ki, ha $i$ helyébe eslőször behelyettesítünk 1-et, akkor $10^1+1$-et kapunk, ha 2-őt, akkor $10^2+1$-et, stb. és aztán ezeket mind összeadjuk.
...
Az összegből először kiemeltem az $i$-t tartalmazó tagokat, így kaptam a két *szummás* tagot, melyből a második egy konstans szám (1) háromszor való összeadása volt (amit elég könnyen kiszámíthatunk). Ugyan ennél az egyenletnél ez felesleges rendezgetésnek tűnhet, de később még hasznos lesz a gyakorlat során.  
Egy másik átrendezős példa:

$$\sum_{i=2}^{4}{i/3} = 2/3+3/3+4/3= 0.667 + 1.000 + 1.333 = 3$$
A gyakorlottabbak a bonyolult tizedes törtekkel való dolgozás helyett így gondolkodtak volna:

$$\sum_{i=2}^{4}{i/3} = 2/3+3/3+4/3 = {2+3+4 \over 3} = {9 \over 3} =3 $$
, azonban egy számítógép nem fog nekiállni átrendezni az egyenleteinket, hanem egyesével kiszámolja a tagokat és összeadja, így kettővel több osztási műveletet végezne el, mint ami feltétlenül szükséges. Így érdemes formailag is átrendezni az egyenletet:
...
Az indexelés bemutatásra került a korábbi fejezetekben. Egy kis ismétlés azonban sosem árt. *Matrix* és *data.frame* indexelése során a táblázat neve után indexelő karaktereket (**[ ]**) írunk, amit egy vesszővel választunk el. A vessző elé az indexelendő sorszámot, míg utána az indexelendő oszlopszámot írjuk: `valtozonev[ sorszam , oszlopszam ]`. Ha a sor- és oszlopszámot üresen hagyjuk, akkor az **R** úgy veszi, mintha mindent ki akarnánk íratni.
...
Logikai müveletekkel pedig megnézhetjük melyik nőstény kutyák marmagassága nagyobb mint 30 cm. Ehhez először is tudnunk kell melyikük nőstények, aztán tudnunk kell melyik nagyobb mint 30 cm és azokat kell látnunk, amikben ez a két tulajdonság egyszerre megvan. Ezt a logikai *ÉS* művelttel tudjuk megtenni.
...
Ha minden egyéb eredményre vagyunk kíváncsiak, tehát minden olyan kutyára, ami vagy hím, vagy nőstény, de alacsonyabb mint 30 cm, akkor a logikai **NEM** operátort használjuk.
