...
Ezzel egyenértékű, ha beírjuk a `y~x` kifejezést a függvény első argomentumaként, mely azt jelöli, hogy *y függvénye x-nek*. A hullámjelet *tilde*-nek is hívják.
...
Átállítani úgy tudjuk, hogy argomentumnak írjuk be a paramétert a `par()` függvénybe és értéket adunk neki. Ha át akarjuk állítani például az ábrázolás színét pirosra, akkor:
...
A `plot()` és `par()` argomentumai közül most is legtöbb használható. Ami különleges: a `labels` argomentumban meghatározhatjuk a különböző kategóriákhoz írt címkéket, a `clockwise = TRUE`-val pedig megváltoztathatjuk a körcikkek irányát.
...
## A konfidencia intervallum számítás logikája
  ...
  1. El kell döntenünk, **milyen eloszlás**t használunk. Ehhez azt kell tudnunk, hogy ismerjük-e a **populáció szórását**. Figyelem: a minta szórását nem elég tudnunk, mert nem arra vagyunk kíváncsiak, hanem a populáció szórására. Ha tehát van egy szórás értékünk, amit egy mintából számoltunk ki, attól még nem fogjuk tudni a populáció szórását! A populáció szórását nagyon kevés helyzetben tudjuk. Ilyen esetek, ha kivettük az összes lehetséges elemet (mintát) így megismertük a populáció valós paramétereit (az ilyen esetek beláthatóan extrém ritkák), vagy ha egy előzetes tanulmány már megbízhatóan megbecsülte azt (irodalmi adatok).    
    _**A.**_ Ha **ismerjük a szórást**, akkor **normális eloszlás** segítségével kiszámíthatjuk a konfidencia intervallumot. Ekkor **nem kell szórást becsülni**, mivel azt már ismerjük.  
    _**B.**_ Ha **nem ismerjük a szórást**, akkor egy olyan eloszlást kell választanunk, amivel megbízhatóan tudjuk közelíteni a normális eloszlást, de nem kell hozzá tudnunk a populáció szórását. Ez a **t-eloszlás**. Mivel nem ismerjük, ezért a **populáció szórását becsülni kell** $\chi^2$ eloszlással.
